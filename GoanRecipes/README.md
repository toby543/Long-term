...
- **App icon & adaptive icon**: `assets/icon.png`, `assets/adaptive-icon.png` (transparent foreground
  for Android's adaptive icon mask), and `assets/splash-icon.png` are generated by
  `scripts/gen_icons.py` (pure Python, no external deps — run `python3 scripts/gen_icons.py` to
  regenerate after changing the palette in `src/theme.ts`).
- **Package name**: `com.goanrecipes.app` (set in `app.json` → `android.package`).
- **Hardware back button**: handled in `App.tsx` — it pops the navigation stack as expected, and
  shows a "Press back again to exit" toast (double-press to exit) when there's nothing left to pop,
  matching standard Android app behavior instead of exiting on the first press.
